...
実際に叩いて結果を表示するタブ式ダッシュボードです。それに加えて、
**「第二のKUSANAGI」**(WordPress高速化サーバー構築キットKUSANAGIのような、
アプリのアップロード後にIPアドレスから起動し、ドメイン登録・HTTPS化を
簡単に自動適用できる運用ツール)を目指し、複数の接続先(aruaru-web用・
他プロジェクト用)を登録・切り替えられる「サイト管理」タブと、IPアドレス
起動・vhost生成・HTTPS(TLS)自動設定/監視/更新の一式を備えます。
...
- **aruaru-dbのバージョン管理系クエリ(ブランチ/ログ/Diff等)やレジストリの
  詳細一覧取得など、DB機能面の深掘りは意図的にスコープ外**。このリポジトリ
  の狙いは「第二のKUSANAGI」的な運用ツール(IPアドレス起動・ドメイン登録の
  簡易化・HTTPS自動化)であり、SQL実行・レジストリ集計という最小限の
  aruaru-db接続機能を超えたDB管理UIの拡張は今後も予定していない。
