# VPSレンタルサーバーへ aruaru-web (と任意で open-web-server) をアップロードし、
...
#   -RemoteAruaruPath    aruaru-webのアップロード先(既定: /root/aruaru-web)
...
param(
    ...
    [string]$VpsUser = "root",
    [int]$VpsPort = 22,
    [string]$RemoteAruaruPath = "/root/aruaru-web",
...
Write-Host "aruaru-web を $VpsUser@${VpsHost}:$RemoteAruaruPath へアップロードします..." -ForegroundColor Cyan
scp -P $VpsPort -r `
    "$RepoRoot\index.html" `
    "$RepoRoot\pkg" `
    "$RepoRoot\scripts" `
